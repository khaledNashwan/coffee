        ...
        <div className=" Proudct_contianer">
          {Products
            ...
            <div className="image_container">
              <img onClick={ () => handelModel(item) } src={item.images} alt={item.title} />
               </div>
              <h3 className="title">{item.title}</h3>
              <Rating rating = {item.rating} reviews ={item.reviews} />
              <p className="price">${item.price} </p>
              <button onClick={ () => handelModel(item) } className='btn_pro' type="submit">more information</button>
            </div>
          ))
            
          }
        </div>
        {openModal && <Model proudctData={proudctData} setOpenModal={setOpenModal} />}
